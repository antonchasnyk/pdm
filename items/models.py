...
class MaterialAsset(models.Model):
    ...
    def __str__(self):
        return '{}, {} [{}]'.format(self.part_number, self.name, self.unit.name)
...
class AmountConstraints(models.Model):
    ...
    def __str__(self):
        return '{} [{}:{}]'.format(self.asset.part_number, self.min_amount, self.max_amount)
...
class Document(models.Model):
    ...
    type = models.ForeignKey(DocumentType, blank=False, null=False, related_name='document', on_delete=models.PROTECT)
    ...
    # TODO exam this way do Two documents for moving assets in one transaction.atomic block
